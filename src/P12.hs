...
parseLine (springsStr:blockStr:_) = Input springsStr (map read (splitOn "," blockStr))
...
unfoldRecord (Input pattern blocks) = Input (intercalate "?" (replicate 5 pattern)) (blocks ++ blocks ++ blocks ++ blocks ++ blocks)
...
countSolutions cache (_:pattern) blocks@(nextBlock:remBlocks) = case M.lookup (pattern, blocks) cache of
        Just x -> (x, cache)
        Nothing ->
            let (count1, cache1) = placeBlock cache pattern (nextBlock-1) remBlocks 
                (count2, cache2) = countSolutions cache1 pattern blocks
            in  (count1 + count2, M.insert (pattern, blocks) (count1 + count2) cache2)
